...
var Application = function(undefined) {
    ...
    var config = {
        ...
        page_dir:       'ajax_pages/',    // page dir (setting to empty string to be able to use absolute URls)
    ...
    var Events = {
        ...
        hashChange: function() {
            ...
            _nav_menu.each(function(index) {
                ...
                if ('#!/' + menu.attr('href').replace(/#/, '').replace(/\.(html|php|asp|jsp)/, '') === window.location.hash) {
    ...
    */
    ...
    this.openPage = function(page_url, menu) {
        if (page_url.indexOf('#') === 0) {
            ...
            window.location.hash = '!/' + page_url.replace(/\.(html|php|asp|jsp)/, '');
            ...
            if (_page_cache[page_url] !== undefined) {
